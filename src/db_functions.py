...
def get_incorrect_classifications(user: User) -> list[WordClassification]:
    ...
    **incorrectly** according to STUDY_PROBLEMS[*study_problem*]["correct"].
    ...
    # Get all classification rows for this user across all sessions
    all_classifications = (
        db.session.query(WordClassification)
        .join(ToolSession, WordClassification.session_id == ToolSession.session_id)
        .filter(ToolSession.user_uuid == user.user_uuid)
        .order_by(WordClassification.classification_order.desc())
        .all()
    )

    # Group by (session_id, word) to get the latest classification per word per session
    latest_by_session_word = {}
    for row in all_classifications:
        key = (row.session_id, row.word)
        if key not in latest_by_session_word:
            latest_by_session_word[key] = row

    # Now check each latest classification
    for row in latest_by_session_word.values():
        # Use the study problem ID stored on the classification row
        # Fall back to the session's study_problem if not stored
        study_problem_id = getattr(row, 'study_problem_id', None)
        if study_problem_id is None:
            # Fallback: get the session's study_problem
            session = ToolSession.query.get(row.session_id)
            study_problem_id = session.study_problem if session else None
            
        if study_problem_id is None:
            continue  # Skip if we can't determine which problem this was for
            
        # Ensure we have a valid study problem
        if study_problem_id >= len(STUDY_PROBLEMS):
            continue
            
        block = STUDY_PROBLEMS[study_problem_id]
        correct_formula_str = block["correct"]
        if correct_formula_str == "N/A":
            continue  # nothing to grade against

        from ltl.spotutils import is_trace_satisfied
        trace_satisfied = is_trace_satisfied(row.word, correct_formula_str)
        expected = (
...
def submit_classification(
    ...
    # Get the current study problem ID
    current_study_problem_id = study_order[study_pos] if study_pos < len(study_order) else None
    
    print(f"DEBUG: submit_classification - storing study_problem_id: {current_study_problem_id}")
    print(f"DEBUG: study_order: {study_order}, study_pos: {study_pos}")
    ...
    new_wc = WordClassification(
        ...
        study_problem_id=current_study_problem_id,
    )
    ...
    if label != ClassificationLabel.UNSURE:
        if prev_wc and prev_wc.classification != ClassificationLabel.UNSURE:
            ...
            # Get the study problem ID to determine formula type
            session = ToolSession.query.get(session_id)
            
            from ltl.spotutils import is_trace_satisfied
            for expr in CandidateExpression.query.filter_by(session_id=session_id):
                word_in_expr = is_trace_satisfied(word, expr.ltl_formula)
                supports = (
                    word_in_expr
    ...
    candidates_in_play = [
        expr for expr, in_play in get_candidate_formulas(session_id).items() if in_play
    ]
...
def get_candidate_formulas(session_id: int):
    """
    Retrieve all candidate formulas for a session, and mark them 'in play'
    ...
    return {
        expr.ltl_formula: (expr.expression_id not in eliminated_ids)
...
def expression_status(session_id: int) -> tuple[list[CandidateExpression], bool]:
    ...
    where a formula is eliminated only after `ELIMINATION_THRESHOLD` contradictions.
    """
