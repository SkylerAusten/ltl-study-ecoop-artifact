...
## [SP] : TODO: Need to plumb in the correct prolific completion codes.
...
generate_accepted_trace, is_trace_satisfied,
...
## TODO: Some issues here. Skyler, could use your help.
    ...
    for rc in rng.sample(block["candidates"], len(block["candidates"])):
        db.session.add(
            CandidateExpression(
                ...
                ltl_formula=rc,  # Now stores LTL formulas in ltl_formula field
                confidence=0,
            )
        )
...
def get_candidate_list_status(session_id: int) -> list:
    formula_status_with_ids = []

    candidate_formulas = get_candidate_formulas(session_id)

    for idx, (expr, in_play) in enumerate(candidate_formulas.items(), start=1):
        fid = f"F{idx}"  # F1, F2, ... (Formula IDs)
        formula_status_with_ids.append({"id": fid, "expr": expr, "in_play": in_play})

    return formula_status_with_ids
...
def setup_user():
    ...
    if TESTING:
        if not prolific_id:
            ...
            return render_template("enter_pid.html", prolific_id=prolific_id or "anon12345", error=error)
    else:
...
def study():
    ...
    candidate_formulas = get_candidate_formulas(session_id)
    candidates_in_play = [expr for expr, in_play in candidate_formulas.items() if in_play]
    formula_with_ids = []
    expr_to_id = {}
    for idx, (expr, in_play) in enumerate(candidate_formulas.items(), start=1):
        fid = f"F{idx}"
        formula_with_ids.append({"id": fid, "expr": normalizeltl(expr), "in_play": in_play})
        expr_to_id[expr] = fid

    classified_words = get_classified_words(session_id)
    text_description = get_latest_description(session_id)

    def get_correct_formula(session):
        for block in STUDY_PROBLEMS:
            if block["id"] == session.study_problem:
                return block["correct"]
        return None

    correct_formula_str = get_correct_formula(g.tool_session)
    all_candidates_are_incorrect = correct_formula_str is not None and correct_formula_str not in candidate_formulas

    if(len(candidates_in_play) == 0):
        # Redirect to review if no candidates remain
        return redirect(url_for("review", reason="No Survivors"))

    word1, word2 = generate_two_distinguishing_words(candidates_in_play=candidates_in_play, excluded_words=classified_words)

    def matching_ids(word):
        matches = []
        for expr in candidate_formulas:
            try:
                if is_trace_satisfied(word, expr):
                    matches.append(expr_to_id[expr])
            except:
                continue
        return matches
    ...
    return render_template(
        ...
        formula_with_ids=formula_with_ids,
...
def classify():
    ...
    frontend_study_problem_id = data.get("study_problem_id")  # from frontend
    ...
    # Log the study problem information for debugging
    current_study_problem_id = g.user.study_order[g.user.study_pos] if g.user.study_pos < len(g.user.study_order) else None
    print(f"DEBUG: Classification - word: {word}, label: {label}")
    print(f"DEBUG: Frontend study_problem_id: {frontend_study_problem_id}")
    print(f"DEBUG: Backend study_problem_id: {current_study_problem_id}")
    print(f"DEBUG: Session study_problem: {g.tool_session.study_problem}")
    print(f"DEBUG: User study_pos: {g.user.study_pos}, study_order: {g.user.study_order}")

    # Validate that frontend and backend agree on study problem
    if frontend_study_problem_id is not None and current_study_problem_id != frontend_study_problem_id:
        print(f"WARNING: Study problem mismatch! Frontend: {frontend_study_problem_id}, Backend: {current_study_problem_id}")
    ...
    formula_data = get_candidate_list_status(response_session_id)

    # Otherwise stay on /study and calculate new word suggestions.
    return jsonify({"formulas": formula_data})
...
def reclassify():
    ...
    # Log the study problem information for reclassification
    current_study_problem_id = g.user.study_order[g.user.study_pos] if g.user.study_pos < len(g.user.study_order) else None
    print(f"DEBUG: Reclassification - word: {word}, new_label: {new_label}")
    print(f"DEBUG: Backend study_problem_id: {current_study_problem_id}")
    print(f"DEBUG: Session study_problem: {g.tool_session.study_problem}")
    print(f"DEBUG: User study_pos: {g.user.study_pos}, study_order: {g.user.study_order}")
...
def review():
    ...
    REVIEW_REASON_TEXT = {
        ...
        "All Confident": "At least one candidate formula supports all of your classifications so far.",
        "No Survivors": "Every candidate formula has been eliminated.",
    }
...
def reflection():
    ...
    # ---- 1. map study_problem → {description, correct-formula} -----
    meta = {
        blk["id"]: {
            "description": blk["description"],
            "correct": parse_ltl_string(blk["correct"]),
        }
    ...
    for row in wrong_rows:
        # Use the study problem ID from the row if available, fallback to session
        study_problem_id = getattr(row, 'study_problem_id', None)
        if study_problem_id is None:
            study_problem_id = sess_map.get(row.session_id)
            
        if study_problem_id is None or study_problem_id >= len(STUDY_PROBLEMS):
            continue  # Skip invalid study problems
            
        correct_formula_str = STUDY_PROBLEMS[study_problem_id]["correct"]
        
        # Use the same logic as get_incorrect_classifications
        # from ltl.spotutils import is_trace_satisfied
        # expected = "accept" if is_trace_satisfied(row.word, correct_formula_str) else "reject"

        blk = blocks[study_problem_id]
        blk["description"] = meta[study_problem_id]["description"]
        blk["wrong"].append(
            {
...
def quiz():
    ...
    qr = QuizResult(user_uuid=g.user.user_uuid, score=score, details=details)
    db.session.add(qr)
    db.session.commit()
